...
declare -i rseed
filename1="seed_file_920_no_optimal.txt" 
filename2="seed_file_920_with_optimal.txt" 
				...
				do
					for d in ${deletion_factor[@]}
						do
							...
							-K /home/opc/chenjie/RBBM/rbbm_src/labelling_func_src/src/pickled_funcs_720 -n youtube >> "$filename2"
							global_runs=$(( $global_runs+1 ))
							python main.py -U lf -e experiment_results_folders/basics -R true -x ${rseed} -X test_cases_seed.txt -T 0 -s ${n} -r ${c} -G optimal -D ${d} -l critical \
							-K /home/opc/chenjie/RBBM/rbbm_src/labelling_func_src/src/pickled_funcs_720 -n youtube >> "$filename2"
							global_runs=$(( $global_runs+1 ))
