...
class DatabaseEngine():
    ...
    #     "cite": "(citing, cited)",
    #     "test": "(ssn, name, job, salary, country)"
    # }
    ...
    def delete(self,rule,rows):
        ...
        cursor = self.connection.cursor()
        # print(f"ps_delete_query: {ps_delete_query}")
        cursor.executemany(ps_delete_query, rows_to_delete)
        rows_affected = cursor.rowcount
        self.connection.commit()
        cursor.close()
    ...
    def create_tables(self, schema):
        ...
        #     'CREATE TABLE IF NOT EXISTS test (ssn int, name text, job text, salary int, country text);',
        ...
        #     'CREATE TABLE IF NOT EXISTS Delta_test (ssn int, name text, job text, salary int, country text);',
        # ]
    ...
    def load_database_tables(self, lst_names, is_delta=False):
        ...
        for name in lst_names:
            ...
            with open(path+name+".csv") as f:
                ...
                print("COPY " + s_name + schema[s_name] + " FROM STDIN DELIMITER ',' CSV;")
