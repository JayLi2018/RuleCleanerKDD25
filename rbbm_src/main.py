...
def main():
	...
	parser.add_argument('-m','--dc_model_type',  type=str, default='muse',
	  help='the source of labelling function (intro / system generate) (default: %(default)s)')
	...
	parser.add_argument("--run-user-behavior", action="store_true")
	parser.add_argument("--user-behavior-err-rate", type=float, default=0)
	parser.add_argument("--user-behavior-step", type=float, default=0)
	...
	if(args.use_case=='dc'):
		...
		# logger.warning(f"before passing to lf_input, run_amazon={run_amazon}")
		...
		run_user_behavior=args.run_user_behavior,
		user_behavior_err_rate=args.user_behavior_err_rate,
		user_behavior_step = args.user_behavior_step
		)
