...
from rbbm_src.holoclean.utils import dictify_df, NULL_REPR
...
class Dataset:
    ...
    def get_cell_id(self, tuple_id, attr_name):
        ...
        """
        # print(str(tuple_id))
        # print(self.attr_count)
        # print(self.attr_to_idx[attr_name])
        # print(self.attr_to_idx)
        # print(f"tuple_id: {tuple_id}, type: {type(tuple_id)}")
        # print(f"self.attr_count: {self.attr_count}, type: {type(self.attr_count)}")
        # print(f"self.attr_to_idx[attr_name]: {self.attr_to_idx[attr_name]}, type: {type(self.attr_to_idx[attr_name])}")

        vid = int(tuple_id)*self.attr_count + self.attr_to_idx[attr_name]
        return vid
    ...
    def get_repaired_dataset(self):
        ...
        repaired_df = self.raw_data.df.copy(deep=True)
        for tid in repaired_vals:
            for attr in repaired_vals[tid]:
                repaired_df.loc[repaired_df['_tid_'] ==tid , [attr]] = repaired_vals[tid][attr]
